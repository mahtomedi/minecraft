...
public class EmoteCommands {
    public static void register(CommandDispatcher<CommandSourceStack> param0) {
        param0.register(Commands.literal("me").then(Commands.argument("action", MessageArgument.message()).executes(param0x -> {
            PlayerChatMessage var0x = MessageArgument.getSignedMessage(param0x, "action");
            CommandSourceStack var1 = param0x.getSource();
            if (var1.isPlayer()) {
                ServerPlayer var2 = var1.getPlayerOrException();
                var2.getTextFilter().processStreamMessage(var0x.signedContent().getString()).thenAcceptAsync(param3 -> {
                    PlayerList var0xx = var1.getServer().getPlayerList();
                    PlayerChatMessage var1x = var1.getServer().getChatDecorator().decorate(var2, var0x);
                    var0xx.broadcastChatMessage(var1x, param3, var2, ChatType.EMOTE_COMMAND);
                }, var1.getServer());
            } else {
