...
public class ServerPacksSource extends BuiltInPackSource {
    ...
    protected Pack createVanillaPack(PackResources param0) {
        return Pack.readMetaAndCreate("vanilla", VANILLA_NAME, false, param1 -> param0, PackType.SERVER_DATA, Pack.Position.BOTTOM, PackSource.BUILT_IN);
    }
    ...
    public static PackRepository createPackRepository(Path param0) {
        return new PackRepository(new ServerPacksSource(), new FolderRepositorySource(param0, PackType.SERVER_DATA, PackSource.WORLD));
    }

    public static PackRepository createPackRepository(LevelStorageSource.LevelStorageAccess param0) {
        return createPackRepository(param0.getLevelPath(LevelResource.DATAPACK_DIR));
    }
}
