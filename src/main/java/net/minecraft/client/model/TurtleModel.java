package net.minecraft.client.model;

import com.google.common.collect.ImmutableList;
import com.google.common.collect.Iterables;
import com.mojang.blaze3d.vertex.PoseStack;
import com.mojang.blaze3d.vertex.VertexConsumer;
import net.minecraft.client.model.geom.ModelPart;
import net.minecraft.client.model.geom.PartPose;
import net.minecraft.client.model.geom.builders.CubeListBuilder;
import net.minecraft.client.model.geom.builders.LayerDefinition;
import net.minecraft.client.model.geom.builders.MeshDefinition;
import net.minecraft.client.model.geom.builders.PartDefinition;
import net.minecraft.util.Mth;
import net.minecraft.world.entity.animal.Turtle;
import net.minecraftforge.api.distmarker.Dist;
import net.minecraftforge.api.distmarker.OnlyIn;

@OnlyIn(Dist.CLIENT)
public class TurtleModel<T extends Turtle> extends QuadrupedModel<T> {
    private static final String EGG_BELLY = "egg_belly";
    private final ModelPart eggBelly;

    public TurtleModel(ModelPart param0) {
        super(param0, true, 120.0F, 0.0F, 9.0F, 6.0F, 120);
        this.eggBelly = param0.getChild("egg_belly");
    }

    public static LayerDefinition createBodyLayer() {
        MeshDefinition var0 = new MeshDefinition();
        PartDefinition var1 = var0.getRoot();
        var1.addOrReplaceChild(
            "head", CubeListBuilder.create().texOffs(3, 0).addBox(-3.0F, -1.0F, -3.0F, 6.0F, 5.0F, 6.0F), PartPose.offset(0.0F, 19.0F, -10.0F)
        );
        var1.addOrReplaceChild(
            "body",
            CubeListBuilder.create()
                .texOffs(7, 37)
                .addBox("shell", -9.5F, 3.0F, -10.0F, 19.0F, 20.0F, 6.0F)
                .texOffs(31, 1)
                .addBox("belly", -5.5F, 3.0F, -13.0F, 11.0F, 18.0F, 3.0F),
            PartPose.offsetAndRotation(0.0F, 11.0F, -10.0F, (float) (Math.PI / 2), 0.0F, 0.0F)
        );
        var1.addOrReplaceChild(
            "egg_belly",
            CubeListBuilder.create().texOffs(70, 33).addBox(-4.5F, 3.0F, -14.0F, 9.0F, 18.0F, 1.0F),
            PartPose.offsetAndRotation(0.0F, 11.0F, -10.0F, (float) (Math.PI / 2), 0.0F, 0.0F)
        );
        int var2 = 1;
        var1.addOrReplaceChild(
            "right_hind_leg", CubeListBuilder.create().texOffs(1, 23).addBox(-2.0F, 0.0F, 0.0F, 4.0F, 1.0F, 10.0F), PartPose.offset(-3.5F, 22.0F, 11.0F)
        );
        var1.addOrReplaceChild(
            "left_hind_leg", CubeListBuilder.create().texOffs(1, 12).addBox(-2.0F, 0.0F, 0.0F, 4.0F, 1.0F, 10.0F), PartPose.offset(3.5F, 22.0F, 11.0F)
        );
        var1.addOrReplaceChild(
            "right_front_leg", CubeListBuilder.create().texOffs(27, 30).addBox(-13.0F, 0.0F, -2.0F, 13.0F, 1.0F, 5.0F), PartPose.offset(-5.0F, 21.0F, -4.0F)
        );
        var1.addOrReplaceChild(
            "left_front_leg", CubeListBuilder.create().texOffs(27, 24).addBox(0.0F, 0.0F, -2.0F, 13.0F, 1.0F, 5.0F), PartPose.offset(5.0F, 21.0F, -4.0F)
        );
        return LayerDefinition.create(var0, 128, 64);
    }

    @Override
    protected Iterable<ModelPart> bodyParts() {
        return Iterables.concat(super.bodyParts(), ImmutableList.of(this.eggBelly));
    }

    public void setupAnim(T param0, float param1, float param2, float param3, float param4, float param5) {
        super.setupAnim(param0, param1, param2, param3, param4, param5);
        this.rightHindLeg.xRot = Mth.cos(param1 * 0.6662F * 0.6F) * 0.5F * param2;
        this.leftHindLeg.xRot = Mth.cos(param1 * 0.6662F * 0.6F + (float) Math.PI) * 0.5F * param2;
        this.rightFrontLeg.zRot = Mth.cos(param1 * 0.6662F * 0.6F + (float) Math.PI) * 0.5F * param2;
        this.leftFrontLeg.zRot = Mth.cos(param1 * 0.6662F * 0.6F) * 0.5F * param2;
        this.rightFrontLeg.xRot = 0.0F;
        this.leftFrontLeg.xRot = 0.0F;
        this.rightFrontLeg.yRot = 0.0F;
        this.leftFrontLeg.yRot = 0.0F;
        this.rightHindLeg.yRot = 0.0F;
        this.leftHindLeg.yRot = 0.0F;
        if (!param0.isInWater() && param0.isOnGround()) {
            float var0 = param0.isLayingEgg() ? 4.0F : 1.0F;
            float var1 = param0.isLayingEgg() ? 2.0F : 1.0F;
            float var2 = 5.0F;
            this.rightFrontLeg.yRot = Mth.cos(var0 * param1 * 5.0F + (float) Math.PI) * 8.0F * param2 * var1;
            this.rightFrontLeg.zRot = 0.0F;
            this.leftFrontLeg.yRot = Mth.cos(var0 * param1 * 5.0F) * 8.0F * param2 * var1;
            this.leftFrontLeg.zRot = 0.0F;
            this.rightHindLeg.yRot = Mth.cos(param1 * 5.0F + (float) Math.PI) * 3.0F * param2;
            this.rightHindLeg.xRot = 0.0F;
            this.leftHindLeg.yRot = Mth.cos(param1 * 5.0F) * 3.0F * param2;
            this.leftHindLeg.xRot = 0.0F;
        }

        this.eggBelly.visible = !this.young && param0.hasEgg();
    }

    @Override
    public void renderToBuffer(PoseStack param0, VertexConsumer param1, int param2, int param3, float param4, float param5, float param6, float param7) {
        boolean var0 = this.eggBelly.visible;
        if (var0) {
            param0.pushPose();
            param0.translate(0.0F, -0.08F, 0.0F);
        }

        super.renderToBuffer(param0, param1, param2, param3, param4, param5, param6, param7);
        if (var0) {
            param0.popPose();
        }

    }
}
