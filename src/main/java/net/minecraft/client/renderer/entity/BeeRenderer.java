...
public class BeeRenderer extends MobRenderer<Bee, BeeModel<Bee>> {
    ...
    public BeeRenderer(EntityRendererProvider.Context param0) {
        super(param0, new BeeModel<>(param0.bakeLayer(ModelLayers.BEE)), 0.4F);
    }

    public ResourceLocation getTextureLocation(Bee param0) {
        if (param0.isAngry()) {
            return param0.hasNectar() ? ANGRY_NECTAR_BEE_TEXTURE : ANGRY_BEE_TEXTURE;
        } else {
            return param0.hasNectar() ? NECTAR_BEE_TEXTURE : BEE_TEXTURE;
        }
    }
}
