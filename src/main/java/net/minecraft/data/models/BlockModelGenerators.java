...
public class BlockModelGenerators {
    ...
    private void createCampfire() {
        ResourceLocation var0 = ModelLocationUtils.getModelLocation(Blocks.CAMPFIRE, "_off");
        ResourceLocation var1 = ModelLocationUtils.getModelLocation(Blocks.CAMPFIRE);
        this.createSimpleFlatItemModel(Items.CAMPFIRE);
        this.blockStateOutput
            .accept(
                MultiVariantGenerator.multiVariant(Blocks.CAMPFIRE)
                    .with(createBooleanModelDispatch(BlockStateProperties.LIT, var1, var0))
                    .with(createHorizontalFacingDispatchAlt())
            );
    }
    ...
    private void createNetherPortalBlock(Block param0) {
        this.blockStateOutput
            .accept(
                MultiVariantGenerator.multiVariant(param0)
                    .with(
                        PropertyDispatch.property(BlockStateProperties.HORIZONTAL_AXIS)
                            .select(Direction.Axis.X, Variant.variant().with(VariantProperties.MODEL, ModelLocationUtils.getModelLocation(param0, "_ns")))
                            .select(Direction.Axis.Z, Variant.variant().with(VariantProperties.MODEL, ModelLocationUtils.getModelLocation(param0, "_ew")))
                    )
            );
    }
    ...
    private void createBookBox() {
        List<ResourceLocation> var0 = Stream.of("_a", "_b", "_c", "_d").map(param0 -> {
            TextureMapping var0x = TextureMapping.cube(Blocks.OAK_PLANKS).put(TextureSlot.FRONT, TextureMapping.getBlockTexture(Blocks.BOOK_BOX, param0));
            return ModelTemplates.COMMAND_BLOCK.createWithSuffix(Blocks.BOOK_BOX, param0, var0x, this.modelOutput);
        }).collect(Collectors.toList());
        Variant[] var1 = var0.stream().map(param0 -> Variant.variant().with(VariantProperties.MODEL, param0)).toArray(param0 -> new Variant[param0]);
        this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.BOOK_BOX, var1).with(createHorizontalFacingDispatch()));
        this.delegateItemModel(Blocks.BOOK_BOX, var0.get(1));
    }
