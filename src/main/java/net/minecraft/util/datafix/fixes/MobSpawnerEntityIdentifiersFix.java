...
public class MobSpawnerEntityIdentifiersFix extends DataFix {
    ...
    private Dynamic<?> fix(Dynamic<?> param0) {
        if (!"MobSpawner".equals(param0.get("id").asString(""))) {
            ...
            Optional<String> var0 = param0.get("EntityId").asString();
            if (var0.isPresent()) {
                Dynamic<?> var1 = DataFixUtils.orElse(param0.get("SpawnData").get(), param0.emptyMap());
            ...
            Optional<? extends Stream<? extends Dynamic<?>>> var2 = param0.get("SpawnPotentials").asStreamOpt();
            if (var2.isPresent()) {
                param0 = param0.set(
                    "SpawnPotentials",
                    param0.createList(
                        var2.get()
                            .map(
                                param0x -> {
                                    Optional<String> var0x = param0x.get("Type").asString();
                                    if (var0x.isPresent()) {
                                        Dynamic<?> var1x = DataFixUtils.orElse(param0x.get("Properties").get(), param0x.emptyMap())
    ...
    public TypeRewriteRule makeRule() {
        ...
        return this.fixTypeEverywhereTyped("MobSpawnerEntityIdentifiersFix", this.getInputSchema().getType(References.UNTAGGED_SPAWNER), var0, param1 -> {
            ...
            Pair<?, ? extends Optional<? extends Typed<?>>> var1x = var0.readTyped(this.fix(var0x));
            return !((Optional)var1x.getSecond()).isPresent() ? param1 : (Typed)((Optional)var1x.getSecond()).get();
        });
    }
}
