...
public class MinecartTNT extends AbstractMinecart {
    ...
    public void tick() {
        ...
        if (this.horizontalCollision) {
            double var0 = getHorizontalDistanceSqr(this.getDeltaMovement());
            if (var0 >= 0.01F) {
                this.explode(var0);
            }
        }
    ...
    public boolean hurt(DamageSource param0, float param1) {
        ...
        if (var0 instanceof AbstractArrow) {
            AbstractArrow var1 = (AbstractArrow)var0;
            if (var1.isOnFire()) {
    ...
    protected void explode(double param0) {
        if (!this.level.isClientSide) {
            ...
            this.level.explode(this, this.x, this.y, this.z, (float)(4.0 + this.random.nextDouble() * 1.5 * var0), Explosion.BlockInteraction.BREAK);
            this.remove();
        }
