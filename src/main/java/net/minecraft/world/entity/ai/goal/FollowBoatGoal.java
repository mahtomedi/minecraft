...
public class FollowBoatGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        for(Boat var2 : var0) {
            if (var2.getControllingPassenger() != null
                && (Mth.abs(((LivingEntity)var2.getControllingPassenger()).xxa) > 0.0F || Mth.abs(((LivingEntity)var2.getControllingPassenger()).zza) > 0.0F)) {
                var1 = true;
                break;
            }
        }
    ...
    public void start() {
        for(Boat var1 : this.mob.level.getEntitiesOfClass(Boat.class, this.mob.getBoundingBox().inflate(5.0))) {
            if (var1.getControllingPassenger() != null && var1.getControllingPassenger() instanceof LivingEntity) {
                this.following = (LivingEntity)var1.getControllingPassenger();
                break;
            }
        }
    ...
    public void tick() {
        ...
        float var1 = this.currentGoal == BoatGoals.GO_IN_BOAT_DIRECTION ? (var0 ? 0.17999999F : 0.0F) : 0.135F;
        ...
        if (--this.timeToRecalcPath <= 0) {
            ...
            if (this.currentGoal == BoatGoals.GO_TO_BOAT) {
                BlockPos var2 = new BlockPos(this.following).relative(this.following.getDirection().getOpposite());
