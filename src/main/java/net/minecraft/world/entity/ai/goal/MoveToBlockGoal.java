...
public abstract class MoveToBlockGoal extends Goal {
    ...
    protected int nextStartTick(PathfinderMob param0) {
        return reducedTickDelay(200 + param0.getRandom().nextInt(200));
    }
    ...
    public void tick() {
        ...
        if (!var0.closerToCenterThan(this.mob.position(), this.acceptedDistance())) {
            this.reachedTarget = false;
            ++this.tryTicks;
            if (this.shouldRecalculatePath()) {
                this.mob.getNavigation().moveTo((double)var0.getX() + 0.5, (double)var0.getY(), (double)var0.getZ() + 0.5, this.speedModifier);
            }
        } else {
