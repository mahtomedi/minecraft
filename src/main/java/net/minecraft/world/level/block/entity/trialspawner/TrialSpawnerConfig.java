...
public record TrialSpawnerConfig(
    ...
    public static MapCodec<TrialSpawnerConfig> MAP_CODEC = RecordCodecBuilder.mapCodec(
        param0 -> param0.group(
                ...
                .apply(param0, TrialSpawnerConfig::new)
    );

    public int calculateTargetTotalMobs(int param0) {
        return (int)Math.floor((double)(this.totalMobs + this.totalMobsAddedPerPlayer * (float)param0));
    }

    public int calculateTargetSimultaneousMobs(int param0) {
        return (int)Math.floor((double)(this.simultaneousMobs + this.simultaneousMobsAddedPerPlayer * (float)param0));
    }
}
