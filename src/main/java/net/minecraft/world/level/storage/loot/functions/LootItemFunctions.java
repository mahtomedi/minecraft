...
public class LootItemFunctions {
    ...
    private static LootItemFunctionType register(String param0, Codec<? extends LootItemFunction> param1) {
        return Registry.register(BuiltInRegistries.LOOT_FUNCTION_TYPE, new ResourceLocation(param0), new LootItemFunctionType(param1));
    }

    public static BiFunction<ItemStack, LootContext, ItemStack> compose(List<? extends BiFunction<ItemStack, LootContext, ItemStack>> param0) {
        List<BiFunction<ItemStack, LootContext, ItemStack>> var0 = List.copyOf(param0);

        return switch(var0.size()) {
            case 0 -> IDENTITY;
            case 1 -> (BiFunction)var0.get(0);
            case 2 -> {
                BiFunction<ItemStack, LootContext, ItemStack> var1 = var0.get(0);
                BiFunction<ItemStack, LootContext, ItemStack> var2 = var0.get(1);
                yield (param2, param3) -> var2.apply(var1.apply(param2, param3), param3);
            }
            default -> (param1, param2) -> {
            for(BiFunction<ItemStack, LootContext, ItemStack> var0x : var0) {
