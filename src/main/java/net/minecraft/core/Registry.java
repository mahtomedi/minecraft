...
public abstract class Registry<T> implements IdMap<T> {
    ...
    public static final Registry<StructurePoolElementType> STRUCTURE_POOL_ELEMENT = registerSimple(
        "structure_pool_element", () -> StructurePoolElementType.EMPTY
    );
    ...
    public static final DefaultedRegistry<MemoryModuleType<?>> MEMORY_MODULE_TYPE = registerDefaulted(
        "memory_module_type", "dummy", () -> MemoryModuleType.DUMMY
    );
    ...
    private static <T> Registry<T> registerSimple(String param0, Supplier<T> param1) {
        return internalRegister(param0, new MappedRegistry<>(), param1);
    }

    private static <T> DefaultedRegistry<T> registerDefaulted(String param0, String param1, Supplier<T> param2) {
    ...
    private static <T, R extends WritableRegistry<T>> R internalRegister(String param0, R param1, Supplier<T> param2) {
        ...
        return REGISTRY.register(var0, param1);
    }
    ...
    public static <T> T register(Registry<? super T> param0, ResourceLocation param1, T param2) {
        return ((WritableRegistry)param0).register(param1, param2);
    }

    public static <T> T registerMapping(Registry<? super T> param0, int param1, String param2, T param3) {
        return ((WritableRegistry)param0).registerMapping(param1, new ResourceLocation(param2), param3);
    }

    static {
        LOADERS.entrySet().forEach(param0 -> {
            if (param0.getValue().get() == null) {
                LOGGER.error("Unable to bootstrap registry '{}'", param0.getKey());
            }

        });
        REGISTRY.forEach(param0 -> {
            if (param0.isEmpty()) {
                LOGGER.error("Registry '{}' was empty after loading", REGISTRY.getKey(param0));
                if (SharedConstants.IS_RUNNING_IN_IDE) {
                    throw new IllegalStateException("Registry: '" + REGISTRY.getKey(param0) + "' is empty, not allowed, fix me!");
                }
            }

            if (param0 instanceof DefaultedRegistry) {
                ResourceLocation var0 = ((DefaultedRegistry)param0).getDefaultKey();
                Validate.notNull(param0.get(var0), "Missing default of DefaultedMappedRegistry: " + var0);
            }
